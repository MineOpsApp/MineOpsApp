...
import { createGuestAccount } from '../../services/api';
...
export function SupervisorGuestScreen({ session }: Props) {
  ...
  async function createGuest() {
    ...
    try {
      ...
      if (msg.includes('409')) {
        ...
        Alert.alert('Failed', 'Could not create guest account.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      <Text style={styles.pageTitle}>Create Guest Account</Text>
      <Text style={styles.pageSub}>Create a time-limited account for a site visitor</Text>
        ...
        <>
          ...
          <View style={styles.card}>
            ...
            {GUEST_TYPES.map((gt) => (
              <Pressable
                key={gt.id}
                onPress={() => setGuestType(gt.id)}
                style={[styles.typeRow, guestType === gt.id && { borderColor: gt.color, backgroundColor: `${gt.color}11` }]}
              >
...
const styles = StyleSheet.create({
  ...
  fieldLabel: { color: '#5d6875', fontSize: 11, fontWeight: '800', letterSpacing: 0.5, marginBottom: 6, textTransform: 'uppercase' },
